...
public class SparkUtil {
  ...
  public boolean sparkStickyFault = false;

  /** Processes a value from a Spark only if the value is valid. */
  public void ifOk(SparkBase spark, DoubleSupplier supplier, DoubleConsumer consumer) {
  ...
  /** Processes a value from a Spark only if the value is valid. */
  public void ifOk(SparkBase spark, DoubleSupplier[] suppliers, Consumer<double[]> consumer) {
  ...
  public double ifOkOrDefault(SparkBase spark, DoubleSupplier supplier, double defaultValue) {
    double value = supplier.getAsDouble();
    if (spark.getLastError() == REVLibError.kOk) {
      return value;
    } else {
